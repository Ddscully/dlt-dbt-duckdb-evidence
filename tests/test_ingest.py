...
from ingest import fixtures, http, pipeline
from ingest.sources import ecb, eurostat, retail, weather, worldbank
...
def test_wdi_url_asks_for_the_whole_series_by_default():
    assert "date=" not in worldbank.wdi_url("SP.POP.TOTL")


def test_wdi_url_adds_the_date_window():
    url = worldbank.wdi_url("SP.POP.TOTL", 1, 2021)
...
def test_wdi_start_year_is_a_lookback_window_not_the_watermark():
    ...
    assert worldbank.wdi_start_year(None) is None  # first load: everything
    assert worldbank.wdi_start_year(2025) == 2025 - worldbank.WDI_LOOKBACK_YEARS + 1
...
def _serve_wdi(monkeypatch, rows_by_indicator: dict[str, list[dict]]) -> list[str]:
    ...
    monkeypatch.setattr(http, "get_json", fake_get_json)
    monkeypatch.setattr(
        worldbank, "WB_WDI_INDICATORS", {code: code.lower() for code in rows_by_indicator}
    )
    return calls
...
def test_wb_wdi_first_load_fetches_everything_and_records_a_watermark(monkeypatch):
    ...
    assert state[worldbank.WDI_WATERMARK_KEY] == {"SP.POP.TOTL": 2024}
...
def test_wb_wdi_incremental_load_asks_only_for_the_lookback_window(monkeypatch):
    ...
    list(worldbank.wb_wdi())
    assert all(f"&date={2025 - worldbank.WDI_LOOKBACK_YEARS + 1}:" in url for url in calls)
...
def test_wb_wdi_partition_window_is_asked_for_verbatim(monkeypatch):
    ...
    list(worldbank.wb_wdi((1990, 1995)))
...
def test_wb_wdi_backfill_does_not_move_the_watermark(monkeypatch):
    ...
    list(worldbank.wb_wdi((2020, 2025)))
    assert state[worldbank.WDI_WATERMARK_KEY] == {}
...
def test_wb_wdi_rejects_error_object_served_with_200(monkeypatch):
    ...
    monkeypatch.setattr(worldbank, "WB_WDI_INDICATORS", {"NOPE": "nope"})
...
def test_eu_elec_prices_walks_the_grid(monkeypatch):
    ...
    rows = list(eurostat.eu_elec_prices())
    assert rows == [
...
def test_eu_elec_prices_skips_absent_cells(monkeypatch):
    ...
    monkeypatch.setattr(http, "get_json", lambda url, **kw: _jsonstat({"0": 0.25}))
    rows = list(eurostat.eu_elec_prices())
    assert len(rows) == 1
...
def test_fx_start_date_is_the_series_start_until_a_watermark_exists():
    assert ecb.fx_start_date(None) == ecb.FX_FIRST_DATE
...
def test_fx_start_date_looks_back_from_the_watermark():
    ...
    assert ecb.fx_start_date("2026-08-07") == "2026-07-29"
...
def test_fx_start_date_cannot_ask_for_dates_before_the_euro():
    ...
    assert ecb.fx_start_date("1999-01-05") == ecb.FX_FIRST_DATE
...
def test_ecb_fx_rates_advances_the_watermark_to_the_newest_date(monkeypatch):
    ...
    list(ecb.ecb_fx_rates())
    assert state[ecb.FX_WATERMARK_KEY] == "2024-01-10"
...
def test_ecb_fx_rates_leaves_the_watermark_alone_on_an_empty_response(monkeypatch):
    ...
    """
    state = _state(monkeypatch, {ecb.FX_WATERMARK_KEY: "2024-01-10"})
    _serve_fx(monkeypatch, _fx_payload({}))

    assert list(ecb.ecb_fx_rates()) == []
    assert state[ecb.FX_WATERMARK_KEY] == "2024-01-10"


def test_ecb_fx_rates_asks_for_the_lookback_window_when_it_has_a_watermark(monkeypatch):
    _state(monkeypatch, {ecb.FX_WATERMARK_KEY: "2026-08-07"})
...
def retail_con(monkeypatch, tmp_path):
    ...
    con.execute(f"create or replace view sheets as {workbook.sheets_sql(retail.retail_workbook())}")
    return con
...
def test_retail_fixture_and_live_caches_never_share_a_path(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("INGEST_CACHE_DIR", str(tmp_path))
    monkeypatch.setenv(fixtures.ENV_VAR, "1")
    fixture_path = retail.retail_workbook()
    monkeypatch.setenv(fixtures.ENV_VAR, "0")
    live_dir = Path(retail.cache_dir()) / "live"
    assert fixture_path.parent != live_dir
    assert not (live_dir / retail.RETAIL_WORKBOOK_NAME).exists()
...
def test_retail_cache_notices_a_re_recorded_archive(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(retail.fixtures, "path_for", lambda url: archive)

    def record(payload: bytes) -> None:
        with zipfile.ZipFile(archive, "w") as zf:
            zf.writestr(retail.RETAIL_WORKBOOK_NAME, payload)

    record(b"the first recording")
    first = retail.retail_workbook()
    assert first.read_bytes() == b"the first recording"
    # Cached: same archive, same path, no second extraction.
    assert retail.retail_workbook() == first

    record(b"a re-recorded slice")
    second = retail.retail_workbook()
    assert second != first
...
def test_retail_line_number_is_stable_across_reads(retail_con):
    ...
    read = lambda: retail_con.sql(  # noqa: E731
        f"""select invoice, line_number, stock_code, quantity
            from ({retail.retail_sql()}) order by invoice, line_number"""
    ).fetchall()
    assert read() == read()
...
def test_retail_month_filter_is_the_partition(retail_con):
    ...
    all_rows = retail_con.sql(f"select count(*) from ({retail.retail_sql()})").fetchone()[0]
    window = retail.retail_sql(("2010-01", "2010-03"))
    ...
    per_month = sum(
        retail_con.sql(f"select count(*) from ({retail.retail_sql((m, m))})").fetchone()[0]
...
def test_retail_month_filter_never_splits_an_invoice(retail_con):
    ...
    straddling = retail_con.sql(
        f"""select count(*) from (
                select invoice from ({retail.retail_sql()})
...
def test_retail_partition_bounds_match_the_data(retail_con):
    ...
    lo, hi = retail_con.sql(
        f"select min(invoice_month), max(invoice_month) from ({retail.retail_sql()})"
    ).fetchone()
    assert lo >= retail.RETAIL_FIRST_MONTH
    assert hi <= retail.RETAIL_LAST_MONTH
...
def test_weather_call_units_matches_open_meteos_published_formula():
    ...
    """
    assert weather.weather_call_units(1, 14, variables=10) == pytest.approx(1.0)
    assert weather.weather_call_units(1, 31_412, variables=3) == pytest.approx(673.1, abs=0.1)
    # The default is the configured variable list, not a hardcoded count.
    assert weather.weather_call_units(41, 365) == pytest.approx(
        len(weather.WEATHER_DAILY_VARIABLES) / 10 * (365 / 14) * 41
    )
...
def test_weather_locations_are_sorted_and_scoped(monkeypatch):
    ...
    monkeypatch.setattr(http, "get_json", lambda url, **kw: payload)
    monkeypatch.setattr(weather, "WEATHER_COUNTRIES", ("DEU", "POL"))

    assert weather.weather_locations() == [
        ("DEU", 52.5235, 13.4115),
        ("POL", 52.26, 21.02),
    ]
...
def test_weather_end_date_stops_short_of_the_archives_edge():
    ...
    end = weather.weather_end_date(today=date(2026, 8, 27))
    assert end == "2026-08-24"
    assert weather.WEATHER_END_LAG_DAYS >= 2, (
...
def test_weather_start_date_cold_starts_recently_not_at_the_series_floor():
    ...
    """
    assert weather.weather_start_date(None, today=date(2026, 8, 27)) == "2024-01-01"
    # Clamped at the series floor, so a cold start in 2008 cannot ask for 2006.
    assert (
        weather.weather_start_date(None, today=date(2008, 5, 1))
        == f"{weather.WEATHER_FIRST_YEAR}-01-01"
    )
...
def test_a_cold_start_fits_inside_the_hourly_budget():
    ...
    total = sum(
        weather.weather_call_units(
            len(weather.WEATHER_COUNTRIES),
...
def test_weather_lookback_outlives_the_era5t_revision_window():
    ...
    assert weather.WEATHER_LOOKBACK_DAYS >= 60


def test_weather_windows_are_calendar_years_clipped_at_both_ends():
    windows = weather.weather_windows(years=(2020, 2022), watermark=None, today=date(2026, 8, 27))
    assert windows == [
...
def test_weather_windows_start_from_the_watermarks_lookback():
    windows = weather.weather_windows(watermark="2026-08-20", today=date(2026, 8, 27))
...
def test_weather_windows_are_empty_when_the_range_is_entirely_in_the_future():
    ...
    """
    assert weather.weather_windows(years=(2030, 2031), today=date(2026, 8, 27)) == []
    assert weather.weather_windows(watermark="2026-09-30", today=date(2026, 8, 27)) == [
...
def test_weather_rows_match_the_response_to_the_request_by_position():
    ...
    rows = list(weather._weather_rows(payload, locations))
...
def test_weather_rows_accept_the_single_location_object_shape():
    ...
    rows = list(weather._weather_rows(_weather_entry(["2022-01-01"], [3.0]), [("DEU", 1.0, 2.0)]))
...
def test_weather_rows_cover_every_declared_column():
    ...
    dlt without a word."""
    rows = list(weather._weather_rows(_weather_entry(["2022-01-01"], [3.0]), [("DEU", 1.0, 2.0)]))
    assert set(rows[0]) == set(weather.WEATHER_COLUMNS)
...
def test_weather_retry_reads_the_window_off_the_429_message():
    ...
    assert weather.weather_retry_after("Minutely API request limit exceeded.") == 65.0
    assert weather.weather_retry_after("Hourly API request limit exceeded.") == 660.0


class _FakeResponse:
    """Just enough of `requests.Response` for `get_weather_json`."""
...
def _weather_fetch(monkeypatch, responses):
    """Drive `get_weather_json` over a scripted sequence of outcomes.
    ...
    monkeypatch.setattr(weather.fixtures, "enabled", lambda: False)

    limiter = weather.WeightedWindowLimiter(
        weather.WEATHER_RATE_LIMITS, clock=lambda: 0.0, sleep=lambda _: None
    )
    try:
        return weather.get_weather_json("https://example/archive", limiter, 1.0), calls, sleeps
...
def test_every_attempt_is_charged_including_the_one_that_never_answered(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="failed to fetch weather"):
        weather.get_weather_json("https://example/archive", limiter, 7.0)

    assert charged == [7.0] * weather.WEATHER_RETRIES
    assert len(calls) == weather.WEATHER_RETRIES
...
def _eurostat_price_countries() -> set[str]:
    ...
    payload = json.loads(fixtures.path_for(worldbank.WB_COUNTRY_API).read_text())
    by_iso2 = {
        row["iso2Code"]: row["id"]
...
def test_weather_countries_are_exactly_the_ones_eurostat_prices():
    ...
    scoped = set(weather.WEATHER_COUNTRIES)
...
def test_weather_countries_carry_no_duplicates():
    ...
    assert len(weather.WEATHER_COUNTRIES) == len(set(weather.WEATHER_COUNTRIES))
