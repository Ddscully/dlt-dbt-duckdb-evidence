...
| [`docs/DATA_QUALITY.md`](./docs/DATA_QUALITY.md) | the 368 dbt tests, the mart contracts, and the groups, exposures and model versions around them |
...
Alongside them, `just dbt-build` runs 368 dbt tests and enforces a schema
...
| **Pipeline** | dlt load times per source, rows and year spans per layer, and all 368 dbt tests with their stored failure counts, from the observability tables that `transform/pipeline_status.py` writes. |
