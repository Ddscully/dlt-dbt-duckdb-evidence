...
**A key two processes share has to be spelled the same way in both.** The retail
source names countries in its own words (`EIRE`, `RSA`, `USA`, `Korea`) while
every other model here keys on ISO3, so the two halves of the warehouse could not
be joined at all. 34 of the 43 labels happen to match the country dimension's
names, which is the trap: joining on name succeeds quietly and drops the other
nine, worth 19,898 lines and £652,387, of which `EIRE` alone is £615,520 and the
retailer's second-largest market. A seed resolves all 43 once, in staging, and
the join is a *left* one for a reason an inner join makes vivid: it deletes the
rows it cannot resolve, so the `relationships` test that exists to catch them
then passes, because the rows were the evidence. Measured by deleting one label
from the seed — 17,866 lines and £615,520 gone, all 90 nodes green.
...
above as a tidy row of ticks. 13 facts, 5 conformed dimensions, and the holes are
...
closed vocabulary, and no ratio-shaped name may be declared summable, which holds
across the tree with no exceptions.
...
argued. Three unit tests fail on it. There are 33 unit tests over twelve models,
...
and no local recipe could reproduce it, because every recipe exported the
variable that hid it.
...
rows, and `information_schema` already knows every table's shape. A test's
verdict is read as dbt computes it: `count(*)` over the failures table is only
dbt's *default*, and taking it literally scored two passing tests as failing
against a build that finished clean.
