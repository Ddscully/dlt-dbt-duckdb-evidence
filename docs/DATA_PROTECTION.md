...
December 2011. No name, no address, nothing to contact anyone with. It is still
...
This page is what was done about that, and it is written to be read in the order
the work went: classify, measure, then decide. The measuring is the part that
changed the design, because it is what shows the obvious answer — hash the id —
...
a name with a classified one and carries no label of its own — which is the rule
...
The interesting question is not "is the identifier masked" but "can a person be
...
every possible id — takes **5 ms**. An unsalted digest is not a pseudonym, it is
...
and all 243,007 anonymous order lines land on one identical pseudonym that is
...
surrogate key is for — and still cannot recover who they are. `just export-data`
generates a throwaway salt locally, because a laptop's copy is not a release and
should not look like one.

## Why the boundary and not the model

The obvious place to mask a column is the staging model that reads it, so that
nothing downstream ever sees the clear value. That does not work here, for two
...
schema in the copy before rewriting, and the difference is not academic:
...
* **44 `dbt_test__audit` tables.** `store_failures` is on project-wide, so every
  ...
  then carries. They are empty today because the tests pass — meaning this leak
...
decisive rather than heuristic, because a five-digit customer number cannot match
that pattern.

## Access control: what DuckDB cannot do

There is no row-level security here, and it is worth being precise about why
rather than listing it as a gap. DuckDB 1.5.5 has no access-control surface at
all — `create role`, `grant`, `create user` and `create policy` are each a
parser error, not an unsupported feature. There is no user to attach a policy to
because there is no concept of a user: whoever holds the file holds everything
in it.

So the enforcement point cannot be the database, and pretending otherwise with a
"restricted" schema that nothing prevents anyone from reading would be theatre.
It is the **boundary** instead — the export, which is the only moment this data
crosses from a machine that has it to a machine that does not. That is a real
architecture and not a workaround: it is where a warehouse with roles would put
...
* **No retention or erasure story.** The extract closed in 2011 and its subjects
  are unreachable by construction — there is nothing to action a request
  against, which is a property of this dataset rather than a design.
