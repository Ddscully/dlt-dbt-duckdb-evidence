...
| **Pipeline** | dlt load times per source, rows and year spans per layer, and all 464 dbt tests with their stored failure counts, from the observability tables that `transform/pipeline_status.py` writes. |
