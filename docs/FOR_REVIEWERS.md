...
**What blocks.** 368 dbt tests run inside `dbt build`, every one with
...
| `just dbt-build` | **26.4 s** | 411 nodes — 26 models, 2 snapshots, 6 seeds, 368 tests; contracts are enforced, which is a `describe` per mart |
