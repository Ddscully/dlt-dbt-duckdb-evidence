...
| `just dbt-build` | **23.1 s** | 531 built nodes — 32 models, 2 snapshots, 7 seeds, 460 data tests and 30 unit tests (dbt's own total of 541 adds the 10 exposures, which it counts but never builds); contracts are enforced, which is a `describe` per mart |
