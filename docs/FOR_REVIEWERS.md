...
**What blocks.** 447 dbt tests run inside `dbt build`, every one with
...
| `just dbt-build` | **23.0 s** | 517 built nodes — 31 models, 2 snapshots, 7 seeds, 447 data tests and 30 unit tests (dbt's own total of 527 adds the 10 exposures, which it counts but never builds); contracts are enforced, which is a `describe` per mart |
